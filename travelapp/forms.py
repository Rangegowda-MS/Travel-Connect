...
class SignUpForm(UserCreationForm):
    ...
    email = forms.EmailField(max_length=254,widget= forms.TextInput(attrs={'class':'d-form form-control'}))
...
class FlightForm(forms.Form):
    source = forms.CharField(
        max_length=20,
        label='SOURCE',
        widget=forms.TextInput(attrs={'class': 'fs-form form-control', 'placeholder': 'Source'}),
        required=False
    )
    destination = forms.CharField(
        max_length=20,
        label='DESTINATION',
        widget=forms.TextInput(attrs={'class': 'fds-forms form-control', 'placeholder': 'Destination'}),
        required=False
    )
    date = forms.DateField(
        ...
        widget=forms.DateInput(attrs={
            ...
            'type': 'date',  # This ensures the date input shows a date picker
            'min': datetime.date.today().strftime('%Y-%m-%d')  # Ensures no past dates are selected
        }),
        required=False
    )
...
class HotelForm(forms.Form):
    city = forms.CharField(
        max_length=20,
        label='CITY',
        widget=forms.TextInput(attrs={'class': 'fs-form form-control', 'placeholder': 'City'}),
        required=False
    )
    date = forms.DateField(
        initial=datetime.date.today,
        ...
        widget=forms.DateInput(attrs={
            'class': 'fd-form form-control',
            'placeholder': 'YYYY-MM-DD',
            'type': 'date',  # Ensures the field uses a date picker
            'min': datetime.date.today().strftime('%Y-%m-%d')  # Prevents selecting past dates
        }),
        required=False
    )
...
class ForgotPasswordForm(forms.Form):
    username = forms.CharField(
        max_length=254,
        widget=forms.TextInput(attrs={'class': 'd-form form-control', 'placeholder': 'Enter your username'})
    )

class ResetPasswordForm(forms.Form):
    new_password = forms.CharField(
        max_length=128,
        widget=forms.PasswordInput(attrs={'class': 'd-form form-control', 'placeholder': 'New Password'})
    )
    confirm_password = forms.CharField(
        max_length=128,
        widget=forms.PasswordInput(attrs={'class': 'd-form form-control', 'placeholder': 'Confirm Password'})
    )
